 ...
 * @notice ✅ UPDATED: Now supports public minting with signatures from backend
...
contract MerchManager is Ownable, ReentrancyGuard {
    ...
    /**
     * @dev Register a new event (admin only)
    ...
    /**
     * @dev ✅ UPDATED: Mint SBT with signature from backend + automatic attestation
     * @param _to The recipient address
     * @param _tokenURI The token metadata URI
     * @param _eventId The event ID for attestation
     * @param _signature The signature from backend issuer (NEW PARAMETER)
     * @return uint256 The minted token ID
     * @return bytes32 The created attestation ID
     * @notice ✅ PUBLIC FUNCTION - Anyone can call with valid signature
     * @notice Frontend calls /verify-code API → Gets signature → Calls this function
     */
    function mintSBTWithAttestation(
        address _to,
        string memory _tokenURI,
        bytes32 _eventId,
        bytes memory _signature  // ✅ NEW: Signature from backend API
        ...
        // ✅ Mint SBT with signature verification (users pay gas)
        uint256 tokenId = basicMerch.mintSBT(
            _to, 
            uint256(_eventId), 
            _tokenURI, 
            _signature  // ✅ Pass real signature from backend
        );
